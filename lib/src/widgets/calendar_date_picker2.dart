...
part '_impl/_month_view.dart';

part '_impl/_month_range_picker.dart';
...
class _CalendarDatePicker2State extends State<CalendarDatePicker2> {
  ...
  void initState() {
    ...
    isOnlyMonthRange = config.isOnlyMonthRange;
  }
  ...
  void _handleDisplayedMonthChanged(
    DateTime date, {
    bool fromYearPicker = false,
  }) {
    _vibrate();
  ...
  Widget _buildPicker() {
    switch (_mode) {
      ...
      case CalendarDatePicker2Mode.month:
        return Padding(
          padding: isOnlyMonthRange
              ? EdgeInsets.zero
              : EdgeInsets.only(top: widget.config.controlsHeight ?? _subHeaderHeight),
          child: isOnlyMonthRange
              ? _MonthView(
                  config: widget.config,
                  key: _dayPickerKey,
                  initialYear: _currentDisplayedMonthDate,
                  selectedDates: _selectedDates,
